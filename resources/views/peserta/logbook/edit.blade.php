        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-header bg-warning text-dark">
                    <h4 class="mb-0">Edit Logbook</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('peserta.logbook.update', $logbook->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="room_id" id="room_id" class="form-select @error('room_id') is-invalid @enderror" required>
                                ...
                                @foreach($rooms as $room)
                                    <option value="{{ $room->room_id }}" 
                                        {{ old('room_id', $logbook->room_id) == $room->room_id ? 'selected' : '' }}>
                                        {{ $room->nama_room }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="aktivitas" 
                                      ...
                                      required>{{ old('aktivitas', $logbook->aktivitas) }}</textarea>
                            @error('aktivitas')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="keterangan" id="keterangan" class="form-select @error('keterangan') is-invalid @enderror" required>
                                <option value="offline_kantor" {{ old('keterangan', $logbook->keterangan) == 'offline_kantor' ? 'selected' : '' }}>Offline Kantor</option>
                                <option value="online" {{ old('keterangan', $logbook->keterangan) == 'online' ? 'selected' : '' }}>Online</option>
                                <option value="sakit" {{ old('keterangan', $logbook->keterangan) == 'sakit' ? 'selected' : '' }}>Sakit</option>
                                <option value="izin" {{ old('keterangan', $logbook->keterangan) == 'izin' ? 'selected' : '' }}>Izin</option>
                                <option value="alpha" {{ old('keterangan', $logbook->keterangan) == 'alpha' ? 'selected' : '' }}>Alpha</option>
                            </select>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-warning">
                                <i class="bi bi-save"></i> Update Logbook
                            </button>
                        </div>
                    </form>
                </div>
